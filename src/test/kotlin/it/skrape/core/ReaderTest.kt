...
internal class ReaderTest {
    ...
    internal fun `can read html from file`() {
        val parsedFile = Reader(File("src/test/resources/files/example.html")).read()
        assertThat(parsedFile.title()).isEqualTo("i'm the title")
    }

    @Test
    internal fun `can read html from file with custom charset`() {
        val parsedFile = Reader(File("src/test/resources/files/example.html"), Charsets.ISO_8859_1).read()
        assertThat(parsedFile.title()).isEqualTo("i'm the title")
    }
